...
const baseUrl = 'http://www.17track.net/';

exports.track = (ids, language) => {
    let useLanguage = language;
    if (!Object.prototype.hasOwnProperty.call(status, useLanguage)) {
        useLanguage = defaultLanguage;
    }
    const result = { carrier: '17track', data: [] };
    return new Promise((resolve, reject) => {
        async.each(ids, (id, callback) => {
            const config = {
                method: 'POST',
                uri: `${baseUrl}restapi/handlertrack.ashx`,
                json: true,
                ...
                headers: {
                    ...
                    Origin: baseUrl,
                    Referer: `${baseUrl}${useLanguage}/track?nums=${id}&fc=0`,
            ...
            request(config, (error, response, body) => {
                if (!error) {
                    const data = body.dat[0].track;
                    result.data.push({
                        id,
                        date: Object.prototype.hasOwnProperty.call(data, 'z0') && data.z0 ? data.z0.a : new Date(),
